...
import { clickCoordinates } from '../handlers/eventHandler';
...
export function contentBeforeGame() {
 ...
 /* ctx.font = text + 'px Arial';
  ...
  ctx.beginPath();
  ctx.globalAlpha = 0.5;
  ctx.moveTo(canvas.width / 2 - 0.85 * guardWidth, canvas.height / 2);
  ctx.lineTo(154, 170);
  ctx.lineTo(160, 140);
  ctx.lineTo(canvas.width / 2 - 0.84 * guardWidth, canvas.height / 2);
  ctx.fillStyle = "red";
  ctx.fill();
